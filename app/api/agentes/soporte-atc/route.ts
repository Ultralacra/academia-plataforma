...
export async function POST(request: NextRequest) {
  ...
  const oaiKey = process.env.XACADEMY_SOPORTE_ATC_API_KEY ?? process.env.OPENAI_API_KEY;
  const oaiModel = process.env.XACADEMY_SOPORTE_ATC_MODEL ?? process.env.OPENAI_MODEL ?? "gpt-4o";
  if (!oaiKey) {
    return new Response(
      JSON.stringify({ error: "XACADEMY_SOPORTE_ATC_API_KEY no configurada" }),
