...
export async function PUT(
  ...
  const metadataId = body?.id != null ? String(body.id).trim() : "";
  if (!metadataId) {
  ...
  const updateBody = {
    ...
    payload: {
      ...payload,
      alumno_codigo: String(payload?.alumno_codigo ?? requestedAlumnoId).trim(),
  ...
  const upstream = await fetch(buildUrl(`/metadata/${encodeURIComponent(metadataId)}`), {
    method: "PUT",
    headers: {
      Authorization: auth,
