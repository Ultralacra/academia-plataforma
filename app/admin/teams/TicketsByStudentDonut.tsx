...
export default function TicketsByStudentDonut({
  ...
  const agg = useMemo(() => {
    const arr = (data || []).map((d) => ({ name: d.name || "Sin Alumno", value: Number(d.count || 0) || 0 }));
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white relative">
      <div className="px-5 py-4 border-b border-gray-100">
        ...
        <p className="text-sm text-gray-500">Top alumnos por volumen de tickets</p>
      </div>
            ...
            >
              {displayAgg.map((seg, i) => (
                <Cell
                  ...
                  onMouseLeave={() => setHoverIndex((prev) => (pinnedIndex === null ? null : prev))}
