...
export default function MembershipContractModal({
  open,
  onOpenChange,
  clientCode,
}: {
  ...
  const [contractType, setContractType] = useState<
    "hotselling-pro" | "hotselling-lite" | null
  >("hotselling-pro");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const path =
          contractType === "hotselling-lite"
            ? `/templates/contrato-hotselling-lite.txt`
            : `/templates/contrato-hotselling-pro.txt`;
  ...
  const buildContractHtml = (
    ...
    cType: string | null,
    _headerDataUrl?: string | null,
    signatureDataUrl?: string | null,
  ) => {
    const today = new Date().toLocaleDateString("es-ES");

    // HOTSELLING PRO: construir HTML sin cabecera (header removed)
    if (cType === "hotselling-pro") {
      const sig = formatSignatureDate();
      const startDisplay = formatDateToDisplay(fields.start_date);
      const endDisplay = formatDateToDisplay(fields.end_date);
      ...
      const html =
        ...
        `<p>De una parte, MHF GROUP LLC, con EIN 85-4320656, con domicilio a efectos de notificaciones en 13728 LAGOON ISLE WAY APT 205 ORLANDO, FL 32824, UNITED STATE, quien en adelante se denominará <strong>LA EMPRESA</strong>. Y, de otra parte, <strong>${escapeHtml(fields.client_name || "_____________________________________________")}</strong>, con número de identificación <strong>${escapeHtml(fields.client_id || "____________________")}</strong>, domiciliado(a) en <strong>${escapeHtml(fields.client_address || "___________________")}</strong>, quien en adelante se denominará <strong>EL CLIENTE</strong>, se celebra el presente OTROSÍ al Contrato de Prestación de Servicios correspondiente al programa “HOTSELLING PRO” (en adelante, el Contrato Base), el cual se regirá por las siguientes cláusulas:</p>` +
        ...
        `<div style="text-align:left;">LA CLIENTE (ESTUDIANTE)<br/><br/>Firma: __________________________<br/>Nombre: <strong>${escapeHtml(fields.client_name || "")}</strong><br/>Identificación: ${escapeHtml(fields.client_id || "")}</div>` +
        `</div></div></body></html>`;

      return html;
    }

    // HOTSELLING LITE: construir HTML específico para la opción LITE
    if (cType === "hotselling-lite") {
      const sig = formatSignatureDate();
      const startDisplay = formatDateToDisplay(fields.start_date);
      const endDisplay = formatDateToDisplay(fields.end_date);
      ...
      const liteHtml =
        ...
        `<p>De una parte, MHF GROUP LLC, con EIN 85-4320656, con domicilio a efectos de notificaciones en 13728 LAGOON ISLE WAY APT 205 ORLANDO, FL 32824, UNITED STATE, quien en adelante se denominará <strong>LA EMPRESA</strong>. Y, de otra parte, <strong>${escapeHtml(fields.client_name || "_____________________________________________")}</strong>, con número de identificación <strong>${escapeHtml(fields.client_id || "____________________")}</strong>, domiciliado(a) en <strong>${escapeHtml(fields.client_address || "___________________")}</strong>, quien en adelante se denominará <strong>EL CLIENTE</strong>, se celebra el presente OTROSÍ al Contrato de Prestación de Servicios correspondiente al programa “HOTSELLING LITE” (en adelante, el Contrato Base), el cual se regirá por las siguientes cláusulas:</p>` +
        `<h3 class="clause">PRIMERA. Antecedentes y finalidad</h3>` +
    ...
    // fallback: usar plantilla (si existe) o generar HTML simple
    const tpl = templateText || "";
    const body = tpl || "";
    const full =
      `<div style="font-family: Inter, ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; color:#111; padding:8px; background:#fff">` +
      `<div style="max-width:800px;margin:0 auto;padding:6px;">` +
      `<h2 style="margin:6px 0;">OTROSÍ No. __  AL CONTRATO DE PRESTACIÓN DE SERVICIOS</h2>` +
      `<p><strong>Fecha del acuerdo:</strong> ${today}</p>` +
  ...
  const handleOpenPreview = async () => {
    // fetch signature image and convert to data URL; header removed per user request
    ...
    // both preview and download should show the same document (no header)
  ...
  const handleDownloadFromPreview = () => {
    ...
    a.href = url;
    a.download = `otrosi-hotselling-${clientCode}.html`;
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl p-4">
          <DialogHeader>
            ...
            <DialogDescription>
              Formulario para agregar una membresía y generar el Otrosí.
            </DialogDescription>
          </DialogHeader>
            ...
            {option === "continua" && (
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  <Button
                    variant={
                      contractType === "hotselling-pro" ? "default" : "outline"
                    }
                    onClick={() => setContractType("hotselling-pro")}
                  >
                    Hotselling Pro
                  </Button>
                  <Button
                    variant={
                      contractType === "hotselling-lite" ? "default" : "outline"
                    }
                    onClick={() => setContractType("hotselling-lite")}
                  >
                    Hotselling Lite
                  </Button>
                </div>
                ...
                <div className="mt-4 border p-3 rounded bg-card text-sm text-foreground">
                  <div className="flex items-center justify-between mb-2">
                    <strong>
                      {contractType === "hotselling-lite"
                        ? "Otrosí - HOTSELLING LITE"
                        : "Otrosí - HOTSELLING PRO"}
                    </strong>
    ...
    >
      <DialogContent className="max-w-4xl w-full h-[88vh] overflow-auto p-2">
        <DialogHeader>
          ...
          <DialogDescription>
            Previsualización del Otrosí generado.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="w-full text-sm text-foreground h-[78vh] p-2">
          {editing ? (
            ...
            // use iframe to render HTML preview cleanly
